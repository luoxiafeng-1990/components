...
`BufferComparator` 是一个用于对比验证两个解码器输出的工具类，位于 `productionline/io/` 目录，与 `BufferWriter` 并列，都是Buffer的IO后处理组件。
...
- BufferWriter文档：`packages/components/include/productionline/io/BufferWriter.hpp`
