...
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `components/include/display/SharedDisplayContext.hpp` | **新建** | 共享显示上下文单例 |
| `components/source/display/SharedDisplayContext.cpp` | **新建** | 实现：DMA 分配、BufferPool 管理、timerfd、显示线程 |
| `components/include/display/TacoVOFramebufferDevice.hpp` | **新建** | 继承 LinuxFramebufferDevice 的新类 |
| `components/source/display/TacoVOFramebufferDevice.cpp` | **新建** | 实现：委托 SharedDisplayContext |
| `components/include/display/IDisplayDevice.hpp` | 不变 | 接口不变 |
| `components/include/display/LinuxFramebufferDevice.hpp` | 不变 | 基类不变 |
| `components/source/display/LinuxFramebufferDevice.cpp` | **修改** | 修复 `phys_addr` TODO（使用 taco_sys_get_block） |
| `components/include/productionline/io/IBufferConsumer.hpp` | **修改** | 新增 `shouldRetainBuffer()` 虚方法 |
| `components/source/productionline/io/BufferConsumerStrategies.cpp` | **修改** | DisplayConsumer 新增 `shouldRetainBuffer()` 实现 |
| `components/source/productionline/io/BufferConsumerService.cpp` | **修改** | consumeLoop 支持帧缓存重试 |
| `components/source/display/DisplayDeviceFactory.cpp` | **修改** | 工厂支持创建 TacoVOFramebufferDevice |
