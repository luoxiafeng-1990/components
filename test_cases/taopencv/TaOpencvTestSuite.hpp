...
class TaOpencvTestSuite : public common::ITestModule {
public:
    TaOpencvTestSuite() = default;
    ~TaOpencvTestSuite() override = default;
    
    // ========================================
    // ITestModule 接口实现
    // ========================================
    
    std::string getName() const override { return "taopencv"; }
    std::string getDescription() const override { return "taopencv测试"; }
    ...
    static const std::map<std::string, TaOpencvTestParams>& getPredefinedTests();
    static uint32_t buildConsumeFlags(const WorkerConfig& config);

private:
    bool parseArgs(int argc, char* argv[], WorkerConfig& config, TaOpencvTestParams& params);
