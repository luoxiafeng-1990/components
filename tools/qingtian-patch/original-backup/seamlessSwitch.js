...
async function injectWorkbench() {
    ...
    let content;
    // 从干净的备份开始（如果有的话），避免重复注入
    if (fs.existsSync(bp)) {
        const bc = fs.readFileSync(bp, 'utf-8');
        if (!bc.includes('__QINGTIAN_SEAMLESS__')) {
            content = bc;
        }
        else {
            content = fs.readFileSync(wp, 'utf-8');
        }
    }
    else {
        content = fs.readFileSync(wp, 'utf-8');
        // 确保当前文件是干净的再备份
        if (!content.includes('__QINGTIAN_SEAMLESS__')) {
    ...
    // 步骤2: hook AuthService — 暴露到 window
    const t2 = 'addLoginChangedListener(e){this.loginChangedListeners.push(e)}';
    if (content.includes(t2)) {
        content = content.replace(t2, 'addLoginChangedListener(e){this.loginChangedListeners.push(e);window.__qtAuthService=this}');
    }
    ...
    if (!composerPatch.ok) {
        return {
            ok: false,
            message: 'ComposerService hook 未命中：当前 Cursor workbench 结构和插件预期不一致，批量会话重试桥接无法启用。'
        };
    }
    // 步骤4: 追加轮询脚本
