    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'getStatus': {
                    try {
                        const { isActivated, getLicenseCountdownStatus } = require('./activation');
                        webviewView.webview.postMessage({
                            command: 'licenseStatus',
                ...
                case 'activateLicense': {
                    const code = msg.code;
                    if (code) {
                        const { activate: activateLicense, getLicenseCountdownStatus } = require('./activation');
                ...
                case 'logout': {
                    const confirmSel = await vscode.window.showWarningMessage(uiText('确定要退出激活吗？退出后需要重新输入激活码。'), { modal: true }, uiText('确认退出'));
...
<body data-theme="light" data-send-mode="${this._initialSendMode}" data-bridge-script="${bridgeScriptPath}" data-onboarding-images='${onboardingImages}'>
    <div id="license-gate" class="license-gate">
        ...
        <p class="license-desc">晴天无限MCP v${version} - 请输入激活码启用插件</p>
        <div class="license-form">
            <input type="text" class="license-input" id="license-code-input" placeholder="QT-XXXX-XXXX-XXXX" autocomplete="off" spellcheck="false" />
            <button type="button" class="primary-button license-btn" id="btn-activate">激活</button>
            <button type="button" class="license-purchase-link purchase-link" id="btn-license-purchase-link">购买链接</button>
        </div>
