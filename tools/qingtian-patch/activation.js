...
function createLocalLicense(plan, days) {
    ...
    return {
        plan,
        code: plan === 'trial' ? 'LOCAL-TRIAL' : 'LOCAL-CRYPTO',
...
function renewAfterCryptoPayment(password) {
    ...
    if (String(password || '').trim() !== expected) {
        return { success: false, message: '续费口令不正确' };
    }
...
async function activate(code) {
    ...
    if (!text) {
        return { success: false, message: '请输入续费口令' };
    }
    const renewed = renewAfterCryptoPayment(text);
    if (renewed.success) {
        return renewed;
    }
    if (/^(TRIAL|试用|START)$/i.test(text)) {
        return startLocalTrial();
    }
    return {
        success: false,
        message: renewed.message || '已取消激活码模式。试用到期后请转账并输入续费口令。'
    };
}
